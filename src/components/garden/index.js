    ...
    render() {
        const populateGrid = (gridPositions) =>
            gridPositions.map((gridPosition) => {
                ...
                const plantExistsInGrid =
                    currentPlantData && currentPlantData.isFilled;

                const props = { plantExistsInGrid, ...currentPlantData };

                return <GridPlant {...props} />;
            });
        ...
        return (
            <div class={s.garden}>
                <div class={s['col-name']}>{columnNames}</div>
                <div class={s['row-name-borders']}>{rowNamesGarden}</div>
                <div class={s['row-name-decking']}>{rowNamesDecking}</div>
